...
export const ReportFoundPet = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("found_pets").insert({
      ...
      image_url: imageUrl,
      health_report_url: null, // Explicitly setting to null
    });
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <Label>Species *</Label>
            <Input required value={formData.petSpecies} onChange={e => setFormData({...formData, petSpecies: e.target.value})} placeholder="Dog, Cat, Bird, etc." />
          </div>

          <div>
            <Label>Description *</Label>
            <Textarea required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Describe the pet you found (color, size, distinctive features, condition)" rows={4} />
          </div>
