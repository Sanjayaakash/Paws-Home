...
export const ReportLostPet = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("lost_pets").insert({
      ...
      image_url: imageUrl,
      health_report_url: null, // Explicitly setting to null since field is removed
    });

    if (error) {
      toast.error("Failed to report lost pet");
    } else {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid md:grid-cols-2 gap-4">
            <div>
              ...
              <Input required value={formData.petName} onChange={e => setFormData({...formData, petName: e.target.value})} />
            </div>
            <div>
              <Label>Species *</Label>
              <Input required value={formData.petSpecies} onChange={e => setFormData({...formData, petSpecies: e.target.value})} placeholder="Dog, Cat, etc." />
            </div>
            <div>
              <Label>Breed *</Label>
              <Input required value={formData.petBreed} onChange={e => setFormData({...formData, petBreed: e.target.value})} />
            </div>
            <div>
              <Label>Age</Label>
            ...
            <div>
              ...
              <Input type="number" step="0.01" value={formData.rewardAmount} onChange={e => setFormData({...formData, rewardAmount: e.target.value})} />
            </div>
          </div>

          <div>
            <Label>Description *</Label>
            <Textarea required value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Describe your pet and when/where it was last seen" rows={4} />
          </div>
